...
h.CreateSampleButton = uibutton(h.myfig, ...
    "Text","Create Sample", ...
    "Position",[250 823 100 25],...
    "ButtonPushedFcn", @createsamplecallback);
...
h.BinSizeEditField = uieditfield(h.myfig, ...
    "numeric",...
...
h.LoadImageButton = uibutton(h.myfig, ...
    "Text","Load 2D image(s)", ...
    "Position",[150 785 120 25],...
    "ButtonPushedFcn", @openfilecallback);

h.FileNameEditField = uieditfield(h.myfig, ...
    "text",...
...
h.LoadGammaDataButton = uibutton(h.myfig, ...
    "Text","Load Gamma Data File", ...
    "Position",[10 715 150 25],...
    "ButtonPushedFcn", @opengammafilecallback);

h.GammaFileNameEditField = uieditfield(h.myfig, ...
    "text",...
    "Position",[15 680 255 25],...
    "Value", "Gamma File Name",...
    "HorizontalAlignment", "center",...
    "Tag", "GammaFilename");
...
h.AlphaEditField = uieditfield(h.myfig, ...
    "text",...
...
h.twothetaminEditField = uieditfield(h.myfig, ...
    "text",...
    "Position",[15 620 70 25],...
    "Value", "",...
    "Placeholder",['2',char(952),' min'],...
    "HorizontalAlignment", "center",...
    "Tag", "twothetaminEditField");

h.twothetamaxEditField = uieditfield(h.myfig, ...
    "text",...
    "Position",[100 620 70 25],...
    "Value", "",...
    "Placeholder",['2',char(952),' max'],...
    "HorizontalAlignment", "center",...
    "Tag", "twothetamaxEditField");

h.ChangetwothetarangeButton = uibutton(h.myfig, ...
    "Text",['Change 2',char(952),' range'], ...
    "Position",[185 620 120 25],...
    "ButtonPushedFcn", @changetwothetarangecallback);
...
h.PeakProminenceEditField = uieditfield(h.myfig, ...
    "numeric",...
...
h.PeakWindowEditField = uieditfield(h.myfig, ...
    "numeric",...
...
h.PeakMinHeightEditField = uieditfield(h.myfig, ...
    "numeric",...
...
h.AbscoeffEditField = uieditfield(h.myfig, ...
    "numeric",...
...
h.SpannKompEditField = uieditfield(h.myfig, ...
    "numeric",...
    "Position",[110 58 60 25],...
    "Value", 1122,...
    "ValueDisplayFormat", "%6.0f",...
    "HorizontalAlignment", "center",...
    "Tag", "SpannKompEditField");

h.ModDataButton = uibutton(h.myfig, ...
    "Text","Modify Data", ...
    "Position",[200 110 120 40],...
    "ButtonPushedFcn", @moddatacallback);

h.FitStessDataButton = uibutton(h.myfig, ...
    "Text","Fit Stress Data", ...
    "Position",[200 65 120 40],...
    "ButtonPushedFcn", @fitstressdatacallback);

h.ModStessDataButton = uibutton(h.myfig, ...
    "Text","Modify Stress Data", ...
    "Position",[200 20 120 40],...
    "ButtonPushedFcn", @modstressdatacallback);
...
h.radiobuttonwavelength = uibuttongroup(h.myfig,"Units","pixels",...
    'BorderType','none',...
    'SelectionChangedFcn', @choosewavelengthcallback ,...
    'Position',[500 150 123 85]);  

h.rb1 = uiradiobutton(h.radiobuttonwavelength,'Position',[10 60 91 15],...
    'Text', 'Ga K-alpha');
h.rb2 = uiradiobutton(h.radiobuttonwavelength,'Position',[10 38 91 15],...
    'Text', 'In K-alpha');
h.rb3 = uiradiobutton(h.radiobuttonwavelength,'Position',[10 16 91 15],...
    'Text', 'In K-beta');
...
h.checkboxplotall = uicheckbox(h.myfig,...
    'Position', [1030 250 200 25],...
    'Text','Plot all Intensity profiles',...
    'ValueChangedFcn', @plotallprofilescallback);
...
h.ExportFitDataButton = uibutton(h.myfig, ...
    "Text","Export Fit Data", ...
    "Position",[1250 400 110 30],...
    "ButtonPushedFcn", @exportfitdatacallback);

h.ExportStressDataButton = uibutton(h.myfig, ...
    "Text","Export Stress Data", ...
    "Position",[1380 400 120 30],...
    "ButtonPushedFcn", @exportstressdatacallback);
...
if strcmp(get(h.SampleFormulaeEditField,'Value'),'Elemental formula')
    ...
    h.ElementalFormula = get(h.SampleFormulaeEditField,'Value');
...
[file,location] = uigetfile('*.tif','Select One or More Files',[General.ProgramInfo.Path,'\Data\Results\Pilatus-2DXRD\'],'MultiSelect', 'on');
...
h.BinSize = get(h.BinSizeEditField,"Value");

set(h.FileNameEditField,"Value",strjoin(string(file),', '))

for k = 1:total_images_selected
    [h.IntensityProfiles{k}, Info] = Conversion_2D_XRD(fullfile(location,file{k}),h.BinSize);
end
...
for k = 1:total_images_selected
    ...
    h.alpha(k) = str2double(cell2mat(regexp(file{k},'(?<=alpha).*(?=-caked.tif)','match')));
end

set(h.AlphaEditField,'Value',strjoin(string(h.alpha),', '))
...
[file,location] = uigetfile('*.txt','Select One or More Files',[General.ProgramInfo.Path,'\Data\Results\Pilatus-2DXRD\'],'MultiSelect', 'on');
if isequal(file,0)
   disp('User selected Cancel');
else
   disp(['User selected ', file]);
...
set(hObj,'Text','Loading','backg',[1 .6 .6]) % Change color of button. 
% The pause (or drawnow) is necessary to make button changes appear.
pause(.01)

set(h.GammaFileNameEditField,"Value",strjoin(string(file),', '))
...
for k = 1:total_images_selected
    if ~iscell(file)
        h.GammaDataFile{k} = fullfile(location,file);
        data{k} = readmatrix(fullfile(location,file));
    else
        h.GammaDataFile{k} = fullfile(location,file{k});
...
set(hObj,'Text','Load Gamma Data File','backg',col)  % Now reset the button features.
...
twothetamin = str2double(get(h.twothetaminEditField,'value'));
twothetamax = str2double(get(h.twothetamaxEditField,'value'));
...
if strcmp(get(h.radiobuttonwavelength.SelectedObject,'Text'),'Ga K-alpha')
    h.plotpeakstheo = xline(h.axesPlotIntensityData,PeakPos{1},'--r',rowsAsStrings{1}, 'LabelVerticalAlignment', 'middle', 'LabelHorizontalAlignment', 'left');
elseif strcmp(get(h.radiobuttonwavelength.SelectedObject,'Text'),'In K-alpha')
...
elseif strcmp(get(h.radiobuttonwavelength.SelectedObject,'Text'),'In K-beta')
...
minProm = get(h.PeakProminenceEditField,'value');
window = get(h.PeakWindowEditField,'value');
minHeight = get(h.PeakMinHeightEditField,"Value");

dataX = h.dataX;
dataY = h.dataY;

for k = 1:size(dataY,2)
    results = findPeaksFromStartValuesMatrix1(dataX{k}, dataY{k}, h.UserPeaks, minHeight, minProm, window);
...
UserPeaksdatanew = [UserPeaksdataTable, [countNonNaN{:}]', ones(length(UserPeaksdataTable),1)];
...
col = get(hObj,'backg');  % Get the background color of the figure.
set(hObj,'Text','Fitting peaks ...','backg',[1 .6 .6]) % Change color of button. 
% The pause (or drawnow) is necessary to make button changes appear.
pause(.01)
...
UserSelectedPeaks = reshape(logical(UserPeaksdata(:,3)),length(UserPeaksdata(:,3))/size(h.results,2),[]);
...
for m = 1:size(dataY,2)
    ...
    fitrangepixel = 20;%get(h.FitPixelRangeEditField,"Value");
    % 
    ...
    for l = 1:size(dataY{m},2)
        ...
        for k = 1:length(PeakPosStart)
            idxChannel = find(ismembertol(dataX{m},PeakPosStart(k),0.0005)==1);
            ...
            [fitresult{l,idxSort},~,residual{l,idxSort},~,~,~,jacobian{l,idxSort}] = lsqcurvefit(gaussEqnFirst, [AmplitudeStart(k) PeakPosStart(k) WidthsStart],Xcorr,Ycorr,...
                [0 PeakPosStart(k)-2 0],...
                [1000 PeakPosStart(k)+2 3],...
    ...
    FitDatatmp = [BinnedGammaSortMat(:) FittedPeakPosSortMat(:) FittedPeakPosErrSortMat(:) FittedPeakAmpSortMat(:) FittedPeakWidthSortMat(:)];
...
% end
% 
% % % Filter FitData for outliers
% for k = 1:size(FitDataMod,1)
%     FitDataMod{k}(isoutlier(FitDataMod{k}(:,2)),:) = [];
% end
...
% for k = 1:size(FitDataMod,1)
%     FitDataMod{k}(find(abs(FitDataMod{k}(:,3))>=0.015),:) = [];
% end
% 
...
PeakPosData = cell2mat(cellfun(@mean,FitDataMod,'UniformOutput',false));
...
if size(FitDataMod,1) == 1
    set(h.Slider,'Min',0);
    set(h.Slider,'Max',1);
    set(h.Slider,'SliderStep',[1 1]);
    set(h.Slider,'Value',1);
else
    set(h.Slider,'Max',size(FitDataMod,1));
    set(h.Slider,'SliderStep',[1/(size(FitDataMod,1)-1) 1/(size(FitDataMod,1)-1)]);
    set(h.Slider,'Value',1);
end

% Set plot data for fitted peak positions
set(h.plotdata,'Xdata',FitDataMod{1}(:,1))
set(h.plotdata,'Ydata',FitDataMod{1}(:,2))
set(h.plotdata,'YNegativeDelta',FitDataMod{1}(:,3))
set(h.plotdata,'YPositiveDelta',FitDataMod{1}(:,3))
...
% Reset the button color
set(hObj,'Text','Start Peak Fit','backg',col)  % Now reset the button features.
...
spannkomp = get(h.SpannKompEditField, "value");
% DEK = get(h.dekdata,"data");
