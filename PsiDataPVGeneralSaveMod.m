function [Psidata] = PsiDataPVGeneralSaveMod(DiffractionLines,SPECFileCheck)
    for k = 1:size(DiffractionLines,1)
        for l = 1:size(DiffractionLines{k},2)
            ...
            Psidata.h(k,l) = cellstr(num2str(0,'%.0f'));
            Psidata.k(k,l) = cellstr(num2str(0,'%.0f'));
            Psidata.l(k,l) = cellstr(num2str(0,'%.0f'));
...
%             Psidata.RingCurrent(k,l) = DiffractionLines{k,1}(l).RingCurrent;
            if SPECFileCheck == 1
                if strcmp(DiffractionLines{k}(l).Measurement.Diffractometer.Name,'ETA3000')
                    ...
                    Psidata.Temperatures1(k,l) = DiffractionLines{k,1}(l).Measurement.RealTime;
                    Psidata.Temperatures2(k,l) = DiffractionLines{k,1}(l).Measurement.RealTime;
                    Psidata.HeatRate(k,l) = DiffractionLines{k,1}(l).Measurement.RealTime;
