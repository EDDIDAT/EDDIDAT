function XRD2DStressAnalysis_modPV()
...
for k = 1:total_images_selected
    try
        ...
        errordlg(sprintf('Fehler beim Laden von Datei:\n%s\n\n%s', ...
            file{k}, ME.message), 'Ladefehler');
        return
    end
end
...
for m = 1:size(dataY,2)
    ...
    % NEU - Pseudo-Voigt: p = [A, x0, fwhm, eta]
    pVoigtEqn = @(p, xdata) p(1) .* ( ...
        p(4) .* (1 ./ (1 + ((xdata - p(2))./(p(3)/2)).^2)) + ...
        (1 - p(4)) .* exp(-log(2) .* ((xdata - p(2))./(p(3)/2)).^2) ...
        );
    h.gaussEqnFirst = pVoigtEqn;  % Name bleibt für Kompatibilität mit Rest der GUI
    ...
    % Diese Zeile hinzufügen neben den anderen Vorallokierungen:
    ...
    for l = 1:size(dataY{m},2)
        ...
        for k = 1:length(PeakPosStart)
            ...
            % 
            % % NEU - Parameter 4 (eta) zusätzlich speichern
            % PeakLocs(l,idxSort)  = fitresult{l,idxSort}(2);   % x0
            % PeakAmp(l,idxSort)   = fitresult{l,idxSort}(1);   % A
            % PeakFWHM(l,idxSort)  = fitresult{l,idxSort}(3);   % FWHM
            % PeakEta(l,idxSort)   = fitresult{l,idxSort}(4);   % eta (Lorentz-Anteil)
            ...
            try
                ...
                % Fehlerberechnung via Jacobi-Matrix
                [~,R]      = qr(jacobian{l,idxSort}, 0);
                Rinv       = R \ eye(size(R));
                diag_info  = sum(Rinv.*Rinv, 2);
                n_res      = length(residual{l,idxSort});
                ...
                % NaN einsetzen — diese Punkte werden später herausgefiltert
                fitresult{l,idxSort} = [];
    ...
    % Nach den Fit-Schleifen, vor der FitDatatmp-Erstellung:
    nanMask = isnan(PeakLocs) | isnan(StdError);
    if any(nanMask(:))
        n_failed = sum(nanMask(:));
        warning('[fitpeakscallback] %d Peak-Fit(s) fehlgeschlagen und herausgefiltert.', n_failed);
    end
    
    % Werte auf 0 setzen damit bestehende Filterlogik greift
    PeakLocs(nanMask)  = 0;
    PeakAmp(nanMask)   = 0;
    PeakFWHM(nanMask)  = 0;
    PeakEta(nanMask)   = 0;
    ...
    % NEU - eta-Spalte hinzufügen
    % Erst FittedPeakEtaSortMat analog zu den anderen aufbauen:
    % FittedPeakEtaSortMat = zeros(size(dataY{m},2), length(PeakPosFiltered));
    % for k = 1:size(PeakEta,2)
    %     for l = 1:length(PeakPosFiltered)
    %         FittedPeakEtaSortMat(strfind(ismembertol(PeakEta(:,k), ...
    %             PeakPosFiltered(l),0.03)', 1), l) = ...
    %             PeakEta(strfind(ismembertol(PeakLocs(:,k), ...
    %             PeakPosFiltered(l),0.03)', 1), k);
    %     end
    % end
    FittedPeakEtaSortMat = zeros(size(dataY{m},2), length(PeakPosFiltered));
    for k = 1:size(PeakEta,2)
    ...
    nCols   = size(FitDatatmp, 2);   % automatisch — passt sich bei weiteren Änderungen an
...
if length(val) ~= length(idxalpha)
    % Export peak positions from user defined data
    col1 = cell2mat(UserPeaksdata(:,1));
    
    % Get unique peak positions and indices
    [uniquePeaks, ~, peakIdx] = unique(col1);
    pos = accumarray(peakIdx, (1:numel(col1))', [], @(x){x});
    
    assignin('base','pos',pos)
    assignin('base','col1',col1)

    % Merge data from FitDataMod that are from the same peak,
    % but from different detector positions and alphas
    MergedFitData = cell(numel(pos), 1);
    
    for i = 1:numel(pos)
        idxPeak = pos{i};                    % <-- umbenannt von idx zu idxPeak
    ...
    % Sort merged data by alpha group
    NewFitData = cell(0);
    for l = 1:size(MergedFitData,1)
        A = MergedFitData{l};
        uniqueAlpha = unique(A(:,7));        % <-- umbenannt von vals zu uniqueAlpha
    ...
    % Bei nur einem Peak: NewFitData ist (nAlpha x 1)
    % Bei mehreren Peaks: NewFitData ist (nAlpha x nPeaks)
    % -> immer korrekt zu Spaltenvektor umformen
    ...
    assignin('base','FitDataModMerged',FitDataModMerged)
else
    h.FitDataMod = FitDataMod;
end
...
% for k = 1:size(h.FitDataMod,1)
%     idxDel = abs(h.FitDataMod{k}(:,2)) <= 0.01;
%     if any(idxDel)
%         % FitDataMod ist eine Matrix -> direkt löschen
%         h.FitDataMod{k}(idxDel,:) = [];
%         % fitresultexport{k} ist ein Cell-Array -> Einträge leeren dann entfernen
...
for k = 1:size(h.FitDataMod,1)
    % Nur Zeilen löschen wo 2theta exakt 0 ist (nicht gefittete Peaks)
    % und zusätzlich die Amplitude auch 0 ist (kein echter Peak)
...
if size(PeakPosData,2) < 2
    errordlg('FitDataMod hat unerwartetes Format - bitte Peak-Fit erneut ausführen.','Fehler');
...
if length(val) ~= length(idxalpha)
    ...
    group   = h.FitDataMod{1}(:,8);
    classes = unique(group);
    assignin('base','FitDataMod_debug', h.FitDataMod{1});
    fprintf('FitDataMod{1} Größe: %d x %d\n', size(h.FitDataMod{1},1), size(h.FitDataMod{1},2));
    idx1 = group == classes(1);
    h.plotdata1 = errorbar(h.axes, h.FitDataMod{1}(idx1,1), h.FitDataMod{1}(idx1,2), h.FitDataMod{1}(idx1,3),'s','Color','b');
    ...
    if length(classes) >= 2
        ...
        % Nur eine Gruppe vorhanden — leeren Plot anlegen als Platzhalter
...
set(h.SliderFittedPeaks, 'Max', max(nPoints, 2));  % min. 2 damit SliderStep > 0
if nPoints > 1
    step = 1 / (nPoints - 1);
else
    step = 1;
end
set(h.SliderFittedPeaks, 'SliderStep', [step step]);
