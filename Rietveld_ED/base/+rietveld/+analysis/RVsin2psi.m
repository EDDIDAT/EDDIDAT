...
%% Konstruktor
	...
	methods (Access = public)
		...
		function obj = RVsin2psi(rvContainer)
...
% 			params = evalin('base', 'params');
        ...
        function dhkl = computedhkl(obj, ep)
            ...
            % oder konstant gehalten wird. gefittet = meas, const. = spec
        ...
        function dhklzero = computedhklzero(obj, ep)
            % Berechnet aus den Energielagen die entsprechenden d-Werte.
...
%             TwoTheta = [obj.getRVContainer().getParam('spec','TwoTheta').value]';
            Psi = [obj.getRVContainer().getParam('spec','Psi').value]';
            DEK_S1 = [obj.getRVContainer().getParam('general','DEK_S1').value];
            DEK_S2 = [obj.getRVContainer().getParam('general','DEK_S2').value];
%             EPos = [ep{1:specSize}]';
            dhkl = obj.computedhkl(ep);
            
            % Hier muss eingefuegt werde, dass es pro Phi-Winkel ein
            % entsprechendes d0 geben muss.
            
            % Berechnung der Ausgleichsgeraden von dhkl vs. sin²Psi
            for i = 1:peakCnt
                m_hkl = fitlm(sind(Psi').^2, dhkl(:,i), 'linear');
                %tbl = anova(m_hkl, 'summary');
                tbl = table2array(m_hkl.Coefficients);
                fitvalues_m(i) = tbl(2,1);  % m*x + n
                fitvalues_n(i) = tbl(1,1);  % m*x + n
                fiterrors_m(i) = tbl(2,2);  % m*x + n
                fiterrors_n(i) = tbl(1,2);  % m*x + n
            end
            
            % Berechnung der dehnungsfreien Richtung
                
            %sigma11 = sigma22 = sigma_parallel
            for i = 1:peakCnt
                sin2psistern(:,i) = -2*DEK_S1(i)./DEK_S2(i); 
            end
            
            % Berechnung von d0 aus der dehnungsfreien Richtung
            
            for p = 1:peakCnt
                    dhklzero(:,p) = fitvalues_n(p) + fitvalues_m(p)*sin2psistern(p);
            end
            
        end
        
        function epshkl = computeepshkl(obj, ep)
            % Berechnet aus den dhkl und dem dhklzero-Werten die Dehnung
            % epsilon.
...
%             specSize = obj.getRVContainer().getSpecSize();
            ...
            % 
            dhkl = obj.computedhkl(ep);
            dhklzero = obj.computedhklzero(ep);
            
            for p = 1:peakCnt
                    epshkl(:,p) = (dhkl(:,p) - dhklzero(p))/dhklzero(p);
            end
            
        end
        
        function sigmahkl = computesigmahkl(obj, ep)
            % Berechnet aus den Energielagen die entsprechenden d-Werte.
...
%             EPos = [ep{1:specSize}]';
            ...
            for i = 1:peakCnt
                ...
                %tbl = anova(m_hkl, 'summary');
                tbl = table2array(m_hkl.Coefficients);
                fitvalues(i) = tbl(2,1);    % tbl(2,1) = Steigung
                fiterrors(i) = tbl(2,2);    % tbl(2,1) = Fehler Steigung
            end
            
            % d0 berechnen
            
%             a0 = [obj.getRVContainer().getParam('phase','LatticeParam1').value];
%             H = [obj.getRVContainer().getParam('general','H').value];
%             K = [obj.getRVContainer().getParam('general','K').value];
%             L = [obj.getRVContainer().getParam('general','L').value];
            
            d0_hkl = obj.computedhklzero(ep);
...
%             end
            ...
            for i = 1:peakCnt
                sigmahkl(:,i) = fitvalues(i)*1/d0_hkl(i)*1/DEK_S2(i);
            end
%             disp(sigmahkl)
            ...
            deltasigmahkl = deltasigma_hkl';
            assignin('base', 'deltasigmahkl', deltasigmahkl);
            % Berechnung der mittleren Eindringtiefe tauavg
            
%             EnergyPosReal = obj.computeEnergyPosReal();
            taureal = obj.computeInformationDepth(ep);
            tau = [taureal{1:specSize}]';
%             tauavg = size(1,peakCnt);
            
            for i = 1:peakCnt
                tauavg(:,i) = (tau(1,i) + tau(specSize,i))./2;  %Problem: tauavg hat 2 Zeilen, warum?
            end
            assignin('base', 'tauavg', tauavg);
            % Plot der Ergebnisse

            figure;
            errorbar(tauavg(1,:),sigmahkl,deltasigma_hkl,'--sk')
...
% 			end
% 			
% 			% Eventuell ist das hier verwendete A_logical nicht das
% 			% richtige. Die Sortierung muss erneut mit den Werten von
% 			% EPositionExpsorted durchgefuehrt werden. Das dabei erhaltene
% 			% A_logical ist korrekt.
...
% 			
% 			if PhiCount > 1
% 				m = SpecSize(1)/PhiCount;
% 				n = size(dspacingExptrans,2)*PhiCount;
% 			
...
% 			
% 			% Beruecksichtigung der Anzahl der Phi-Winkel.
% 			if PhiCount > 1
% 				m = SpecSize(1)/PhiCount;
% 				n = size(Informationdepthsortedtrans,2)*PhiCount;
% 			
% 				Informationdepthsorted = reshape(Informationdepthsortedtrans,m,n);
% 				Y = Informationdepthsorted;
% 			else
% 				Y = Informationdepthsortedtrans;
% 			end
% 			
% % 			Y = Informationdepthsorted;
% 		end
...
% 			end
% 			PsiTmp = reshape(PsiTmp,SpecSize(1,1)/PhiCount,PhiCount);
...
% 			end
% 			PhiTmp = reshape(PhiTmp,SpecSize(1,1)/PhiCount,PhiCount);
% 			if PhiCount > 1
% 				Phi = [repmat(PhiTmp(:,1),1,size(DEK_S1Tmp)),repmat(PhiTmp(:,2),1,size(DEK_S1Tmp))];
% 			else
% 				Phi =  repmat(PhiTmp,1,size(DEK_S1Tmp));
% 			end
% 			% d0 auslesen.
% % 			dzeroTmp = params{1,1}(1,1).d0(:);
% 			dzeroTmp = params.phase(1,1).d0;
...
% 				tau180 = tau(:,3:4:end);
% 				tau270 = tau(:,4:4:end);
% 				tau_all = [tau0, tau90, tau180, tau270];
% 				tau_all(tau_all == 0) = NaN;
% 			end
...
% 			for i = 1:length(dzeroTmp)
% 				if PhiCount < 2
% 					subplot(ceil(length(dzeroTmp)/2), floor(length(dzeroTmp)/2), i)
% 				else
% 					subplot(3, 3, i)
% 				end
% 				
...
% 			elseif PhiCount == 2
% 				plot(sind(Psi(:,1)).^2,d_exp(:,i), 'ks', 'LineWidth', 1.25, 'MarkerSize', 6);
% 				plot(sind(Psi(:,1)).^2,d_exp(:,i+NumberOfhkl), 'kv', 'LineWidth', 1.25, 'MarkerSize', 6);
% 				plot(sind(Psi(:,1)).^2,d_meas(:,i), 'r-', 'LineWidth', 2); 
% 				plot(sind(Psi(:,1)).^2,d_meas(:,i+NumberOfhkl), 'b-', 'LineWidth', 2);
% 			elseif PhiCount == 3
% 				plot(sind(Psi(:,1)).^2,d_exp(:,i), 'ks', 'LineWidth', 1.25, 'MarkerSize', 6);
% 				plot(sind(Psi(:,1)).^2,d_exp(:,i+NumberOfhkl), 'kv', 'LineWidth', 1.25, 'MarkerSize', 6);
% 				plot(sind(Psi(:,1)).^2,d_exp(:,i+(2*NumberOfhkl)), 'kp', 'LineWidth', 1.25, 'MarkerSize', 6);
% 				plot(sind(Psi(:,1)).^2,d_meas(:,i), 'r-', 'LineWidth', 2); 
% 				plot(sind(Psi(:,1)).^2,d_meas(:,i+NumberOfhkl), 'b-', 'LineWidth', 2);
% 				plot(sind(Psi(:,1)).^2,d_meas(:,i+(2*NumberOfhkl)), 'g-', 'LineWidth', 2);
% 			elseif PhiCount == 4
% 				plot(sind(Psi(:,1)).^2,d_exp(:,i), 'ks', 'LineWidth', 1.25, 'MarkerSize', 6);
% 				plot(sind(Psi(:,1)).^2,d_exp(:,i+NumberOfhkl), 'kv', 'LineWidth', 1.25, 'MarkerSize', 6);
% 				plot(sind(Psi(:,1)).^2,d_exp(:,i+(2*NumberOfhkl)), 'kp', 'LineWidth', 1.25, 'MarkerSize', 6);
% 				plot(sind(Psi(:,1)).^2,d_exp(:,i+(3*NumberOfhkl)), 'kh', 'LineWidth', 1.25, 'MarkerSize', 6);
% 				plot(sind(Psi(:,1)).^2,d_meas(:,i), 'r-', 'LineWidth', 2); 
% 				plot(sind(Psi(:,1)).^2,d_meas(:,i+NumberOfhkl), 'b-', 'LineWidth', 2);
% 				plot(sind(Psi(:,1)).^2,d_meas(:,i+(2*NumberOfhkl)), 'g-', 'LineWidth', 2);
% 				plot(sind(Psi(:,1)).^2,d_meas(:,i+(3*NumberOfhkl)), 'm-', 'LineWidth', 2);
% 			end
% 			xlabel('sin²\psi');
...
% 			
% 			if PhiCount == 1
% 				legend('experimental d-sin²\psi distribution', ...
% 					   'recalculated d-sin²\psi distribution', ...
...
% 				legend('experimental d-sin²\psi distribution, \phi = 0°', ...
% 				   'experimental d-sin²\psi distribution, \phi = 90°', ...
% 				   'experimental d-sin²\psi distribution, \phi = 180°', ...
% 				   'recalculated d-sin²\psi distribution, \phi = 0°', ...
% 				   'recalculated d-sin²\psi distribution, \phi = 90°', ...
% 				   'recalculated d-sin²\psi distribution, \phi = 180°', ...
% 				   'Location', 'BestOutside'); % Legende
% 			elseif PhiCount == 4
% 				legend('experimental d-sin²\psi distribution, \phi = 0°', ...
% 				   'experimental d-sin²\psi distribution, \phi = 90°', ...
% 				   'experimental d-sin²\psi distribution, \phi = 180°', ...
% 				   'experimental d-sin²\psi distribution, \phi = 270°', ...
% 				   'recalculated d-sin²\psi distribution, \phi = 0°', ...
% 				   'recalculated d-sin²\psi distribution, \phi = 90°', ...
% 				   'recalculated d-sin²\psi distribution, \phi = 180°', ...
% 				   'recalculated d-sin²\psi distribution, \phi = 270°', ...
% 				   'Location', 'BestOutside'); % Legende
% 			end
% 		end
