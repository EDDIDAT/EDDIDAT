...
A1 = evec(:,find(cond>0));
A = [A1; T*A1];
A4 = A(4)/2-2*A(1)*centroid(1)-A(2)/2*centroid(2);
A5 = A(5)/2-2*A(3)*centroid(2)-A(2)/2*centroid(1);
A6 = A(6)+A(1)*centroid(1)^2+A(3)*centroid(2)^2+...
     A(2)/2*centroid(1)*centroid(2)-A(4)/2*centroid(1)-A(5)/2*centroid(2);
...
% eqt = a.*XY(:,1).^2 + b.*XY(:,1).*XY(:,2) + c.*XY(:,2).^2 + d.*XY(:,1) + e.*XY(:,2) + f;
xmin=0.7*min(XY(:,1));
xmax=1.3*max(XY(:,2));
