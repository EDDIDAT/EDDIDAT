...
% PhiForPlot = join(['PhiForPlot',PlotWindow]);
TemperatureForPlot = join(['TemperatureForPlot',PlotWindow]);
ScansForPlot = join(['ScansForPlot',PlotWindow]);
TimesForPlot = join(['TimesForPlot',PlotWindow]);
...
for k = 1:length(h.Measurement)
    ...
    Timetmp(:,k) = h.Measurement(k).Time;
    Scantmp(:,k) = k;
%     Phitmp(:,k) = h.Measurement(k).SCSAngles.phi;
end
% assignin('base','Measurement',h.Measurement)
% if h.SpecFileCheck == 1
    for k = 1:length(h.Measurement)
        if length(num2str(Timetmp(5,k))) == 1 && length(num2str(Timetmp(6,k))) == 2
            Time{k} = [num2str(Timetmp(4,k)),':',['0',num2str(Timetmp(5,k))],':',num2str(Timetmp(6,k)),' ',num2str(Timetmp(3,k)),'-',num2str(Timetmp(2,k)),'-',num2str(Timetmp(1,k))];
        elseif length(num2str(Timetmp(6,k))) == 1 && length(num2str(Timetmp(5,k))) == 2
            Time{k} = [num2str(Timetmp(4,k)),':',num2str(Timetmp(5,k)),':',['0',num2str(Timetmp(6,k))],' ',num2str(Timetmp(3,k)),'-',num2str(Timetmp(2,k)),'-',num2str(Timetmp(1,k))];
        elseif length(num2str(Timetmp(5,k))) == 1 && length(num2str(Timetmp(6,k))) == 1
            Time{k} = [num2str(Timetmp(4,k)),':',['0',num2str(Timetmp(5,k))],':',['0',num2str(Timetmp(6,k))],' ',num2str(Timetmp(3,k)),'-',num2str(Timetmp(2,k)),'-',num2str(Timetmp(1,k))];
        else
            Time{k} = [num2str(Timetmp(4,k)),':',num2str(Timetmp(5,k)),':',num2str(Timetmp(6,k)),' ',num2str(Timetmp(3,k)),'-',num2str(Timetmp(2,k)),'-',num2str(Timetmp(1,k))];
        end
    end
...
for m = 1:size(idxkeepPeaks,1)
    for k = 1:length(indexPeakToKeep{m})
        ...
        % Read in Temperature
        Params.Temperature{m}(k,:) = DiffractionLinestmp{indexPeakToKeep{m}(k),1}(1,idxkeepPeaks(m)).Measurement.Temperatures(1);
    end
end
...
if length(unique(PhiWinkel{1})) > 4
...
if length(unique(ParamsToFit(1).Phi_Winkel{1})) <= 4
    ...
    idxphi180 = [];
    idxphi270 = [];
    if strcmp(XDataStr,'Phi')
        XDataphi0 = ParamsToFit(1).Phi_Winkel;
        xlabel(h.(Axes),'\phi')
...
if ~isempty(idxphi0)
    if strcmp(XDataStr,'Psi')
        ...
        Psimin = unique(cellfun(@min,XDataphi0));
        Psimax = unique(cellfun(@max,XDataphi0));
        if Psimin >= 0 && max(Psimax) <= 90
            ...
            xlabel(h.(Axes),'\psi')
            h.(Axes).XLim = [round(Psimin,-1) round(Psimax,-1)];
            if (abs(Psimin)+Psimax)/10 < 20
                h.(Axes).XTick = (round(Psimin,-1):10:round(Psimax,-1));
    ...
    elseif strcmp(XDataStr,'Temperature')
        XDataphi0 =  ParamsToFit(idxphi0).Temperature;
        Tempmax = cellfun(@max, XDataphi0);
        if round(max(Tempmax)) == 0
            msgbox('Temperature was not changed during the measurement.','Warning Message','warn')
            ...
            h.(Axes).XLim = [0 round(max(Tempmax))];
            xlabel(h.(Axes),'Temperature [°C]')
        end
...
if ~isempty(idxphi90)
    ...
    elseif strcmp(XDataStr,'Energy')
        ...
        xticks(h.(Axes),'auto')
    elseif strcmp(XDataStr,'Temperature')
        XDataphi90 =  ParamsToFit(idxphi90).Temperature;
        Tempmax = cellfun(@max, XDataphi90);
        if round(max(Tempmax)) == 0
            msgbox('Temperature was not changed during the measurement.','Warning Message','warn')
        else
            h.(Axes).XLim = [0 round(max(Tempmax))];
...
if ~isempty(idxphi180)  
    ...
    elseif strcmp(XDataStr,'Energy')
        ...
        xticks(h.(Axes),'auto')
    elseif strcmp(XDataStr,'Temperature')
        XDataphi180 =  ParamsToFit(idxphi180).Temperature;
        Tempmax = cellfun(@max, XDataphi180);
        if round(max(Tempmax)) == 0
            msgbox('Temperature was not changed during the measurement.','Warning Message','warn')
        else
            h.(Axes).XLim = [0 round(max(Tempmax))];
...
if ~isempty(idxphi270)  
    ...
    elseif strcmp(XDataStr,'Energy')
        ...
        xticks(h.(Axes),'auto')
    elseif strcmp(XDataStr,'Temperature')
        XDataphi270 =  ParamsToFit(idxphi270).Temperature;
        Tempmax = cellfun(@max, XDataphi270);
        if round(max(Tempmax)) == 0
            msgbox('Temperature was not changed during the measurement.','Warning Message','warn')
        else
            h.(Axes).XLim = [0 round(max(Tempmax))];
...
if strcmp(XDataStr,'Phi')
    for i = 1:length(ParamsToFit.Psi_Winkel)
        h.(phi).psiIndex{i} = unique(ParamsToFit.Psi_Winkel{i}, 'stable');
    end

    for k = 1:length(ParamsToFit.Psi_Winkel)
        for i = 1:length(h.(phi).psiIndex{k})
            IndexMin(i) = arrayfun(@(x) find(ParamsToFit.Psi_Winkel{k} == x,1,'first'), h.(phi).psiIndex{k}(i) );
            IndexMax(i) = arrayfun(@(x) find(ParamsToFit.Psi_Winkel{k} == x,1,'last'), h.(phi).psiIndex{k}(i) );
            psiTable{k} = [IndexMin; IndexMax]';
        end
    end	
    ...
    for k = 1:length(ParamsToFit.Psi_Winkel)	
        for i = 1:length(h.(phi).psiIndex{k})
            h.(phi).TablePsi{k,i} = ParamsToFit.Psi_Winkel{k}(psiTable{k}(i,1):psiTable{k}(i,2),:);
%             h.(phi).Tablphiu{k,i} = ParamsToFit.Psi_Winkel{k}(psiTable{k}(i,1):psiTable{k}(i,2),:);
            h.(phi).Tablephi{k,i} = ParamsToFit.Phi_Winkel{k}(psiTable{k}(i,1):psiTable{k}(i,2),:);
            h.(phi).Tabledspacing{k,i} = ParamsToFit.LatticeSpacing{k}(psiTable{k}(i,1):psiTable{k}(i,2),:);
            h.(phi).Tabledspacingdelta{k,i} = ParamsToFit.LatticeSpacing_Delta{k}(psiTable{k}(i,1):psiTable{k}(i,2),:);
            h.(phi).TableIB{k,i} = ParamsToFit.IntegralWidth{k}(psiTable{k}(i,1):psiTable{k}(i,2),:);
            h.(phi).TableIntensity_Int{k,i} = ParamsToFit.Intensity_Int{k}(psiTable{k}(i,1):psiTable{k}(i,2),:);
        end
    end
    ...
    for i = 1:length(h.(phi).psiIndex{valueSlider})
        h.(phi).phiplot{i} = errorbar(h.(Axes),h.(phi).Tablephi{valueSlider,i},h.(phi).Tabledspacing{valueSlider,i}.*0,h.(phi).Tabledspacingdelta{valueSlider,i}.*0,'s','LineStyle','--','Color','k','MarkerSize',12,'MarkerEdgeColor','k','MarkerFaceColor',h.Colors{i});
        % Create label data from psi angles
        h.(phi).leglabel{i} = ['\psi = ',num2str(h.(phi).psiIndex{valueSlider}(i,:))];
    end
%     assignin('base','phi',h.(phi))
    % Create data matrix for legend entries.
    for k = 1:length(h.(phi).phiplot)
