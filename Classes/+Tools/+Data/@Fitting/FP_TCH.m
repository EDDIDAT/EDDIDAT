...
function [FitParam, CI, SE] = FP_TCH(X,Y,Index_Peaks,GammaGauss,GammaLorentz,...
    PeakProps)
...
%% (* Stringenzprüfung *)
    ...
    validateattributes(GammaGauss,{'double'},...
        {'real','scalar','>=',0});
    validateattributes(GammaLorentz,{'double'},...
        {'real','scalar','>=',0});
...
%         p(4) = RelationGaussLorentz;
    ...
    %% (* function_handle *)
        fun = @(p,x)Tools.Science.Math.FF_TCH(...
            x,p(1),p(2),p(3),p(4)); % p(1)=Intensity, p(2)=PeakPosition, p(3)= GammaGauss, p(4)=GammaLorentz
    ...
    %% (* Erstellen eines Summen-Handles *)
        ...
        for i_c = 1:L
            fun = [fun, '+Tools.Science.Math.FF_TCH(x,p(',...
...
%% (* LSQ-Fit *)
    ...
    [FitParam,~,residual,~,~,~,jacobian] = lsqcurvefit(fun,(p(:))',X,Y,...
        repmat([-Inf -Inf 0 0],1,L),...
        repmat([Inf Inf 2 0.5],1,L),...
