...
function [FitParam, CI, SE] = FP_PseudoVoigtETA(X,Y,Index_Peaks,Index_PeaksKa2,RelationGaussLorentz,...
    PeakProps)
...
%         % Wellenlängen Co-ka
        lambdaka1 = 1.78897;
        lambdaka2 = 1.79278;
%         % Berechne d-Wert aus User defined Peak positionen
%         dspacing = lambdaka1./sind(p(2)./2)./2;
%         % Berechne zweitheta für Coka2 - diese Werte werden für den Fit der ka2
%         % Komponente benötigt in der 
%         p(5) = 2.*asind(lambdaka2./(2.*dspacing));

%         p(5) = X(Index_PeaksKa2);

    %% (* function_handle *)
        fun = @(p,x)(Tools.Science.Math.FF_PseudoVoigt(...
            x,p(1),p(2),p(3),p(4)) + Tools.Science.Math.FF_PseudoVoigt(...
            x,p(1)/2,(2.*asind(lambdaka2./(2.*lambdaka1./sind(p(2)/2)./2))),p(3),p(4))); % p(1)=Intensity, p(2)=PeakPosition, p(3)= PeakWidth, p(4)=RelationGaussLorentz
    ...
    %% (* Erstellen eines Summen-Handles *)
        ...
        for i_c = 1:L
            fun = [fun, '+Tools.Science.Math.FF_PseudoVoigt(x,p(',...
                ...
                num2str(i_c*4-3),')/2,p(',num2str(i_c*4-2),'),p(',...
...
%     lambdaka1 = 1.78897;
%     lambdaka2 = 1.79278;
%     assignin('base','pParams',p)
%     assignin('base','PeakProps',PeakProps)
...
%     [FitParam,~,residual,~,~,~,jacobian] = lsqcurvefit(fun,(p(:))',X,Y,...
%         ([0 p(2)-0.15 0 -Inf,0 (2.*asind(lambdaka2./(2.*lambdaka1./sind(p(2)/2)./2)))-0.15 0 -Inf]),...
%         ([Inf p(2)+0.15 1 Inf,Inf (2.*asind(lambdaka2./(2.*lambdaka1./sind(p(2)/2)./2)))+0.15 1 Inf]),...
%         optimset('Display','off'));
...
% %          pro Spalte immer eine Funktion steht, double|[NaN 4] /
% %         CI, Confident-Intervals, double|[size(FitParam)]
% function [FitParam, CI] = FP_PseudoVoigt(X,Y,Index_Peaks,PeakPosBoundarys,RelationGaussLorentz,...
%     PeakProps)
% 
% %% (* Stringenzprüfung *)
%     validateattributes(X,{'double'},{'real','finite','column'});
