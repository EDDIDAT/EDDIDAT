...
function [FitParam, CI, SE] = FP_PseudoVoigt_DoublePeakETA(X,Y,Index_Peaks,Index_PeaksKa2,PeakPosBoundarys,RelationGaussLorentz,...
    PeakProps)
...
%         p(5) = 2.*asind(lambdaka2./(2.*dspacing));
    ...
    %% (* function_handle *)
        fun = @(p,x)(Tools.Science.Math.FF_PseudoVoigt(...
            ...
            x,p(1)/2,p(5),p(3),p(4))); % p(1)=Intensity, p(2)=PeakPosition, p(3)= PeakWidth, p(4)=RelationGaussLorentz
    ...
    %% (* Erstellen eines Summen-Handles *)
        ...
        for i_c = 1:L
            fun = [fun, '+Tools.Science.Math.FF_PseudoVoigt(x,p(',...
                num2str(i_c*5-4),'),p(',num2str(i_c*5-3),'),p(',...
                num2str(i_c*5-2),'),p(',num2str(i_c*5-1),'))','+Tools.Science.Math.FF_PseudoVoigt(x,p(',...
                num2str(i_c*5-4),')/2,p(',num2str(i_c*5),'),p(',...
                num2str(i_c*5-2),'),p(',num2str(i_c*5-1),'))'];
        end
        %Zusammenfügen
        fun = str2func([fun,')']);
