...
% + Messserie bzw. Dateiname (#F)
    Index_tmp = Tools.StringOperations.SearchString(M,Diffractometer.ImportLiterals.FileName);
...
%     assignin('base','Scans',Scans)
    ...
    if Diffractometer.CheckSPECformat == 1
        ...
        Index_tmp = Tools.StringOperations.SearchString(M,Diffractometer.ImportLiterals.MotorNames);
...
%             etaneu = reshape(etaPrepmat',1,[]);
        ...
        %             Table{i} = Tabletmp;
            ...
            Index_tmpNoD = Tools.StringOperations.SearchString(M,'#@MCADEV 1');
...
%% (* Durchlaufen aller Scans und Erstellen der Messobjekte*)
    ...
    %% (* Analyse je nach Scan-Typ *)
        ...
        elseif any(strcmp(ScanType,{'mcaacq','twinmcaacq','loopscan'}))
            if strcmp(Diffractometer.Name,'LEDDI')
