...
function obj = LoadFromSpecFile_corr(Filename,Diffractometer,DiffMode,Calibration)
...
%2. Phi-Winkel
    ...
    if size(Index_tmp,1) == 0; phiP = zeros(Count_Scans,1); end
...
%3. Psi-Winkel
    if diffractometerMode == 1
        ...
        if size(Index_tmp,1) == 0; psiP = zeros(Count_Scans,1); end
...
%         Energies_tmp = linspace(obj.ChannelRange(1),obj.ChannelRange(2),obj.ChannelRange(2)-1);
    % + Energien berechnen - neu 2015
        ...
        elseif strcmp(calib,'September 2015')
            ...
            CalibParam_b = 0.00819 - 5.16723e-10*obj.DeadTime + 2.37589e-12*obj.DeadTime.^2;
            CalibParam_c = -1.8617e-9 - 3.7839e-25*obj.DeadTime;
        % Januar 2016
        elseif strcmp(calib,'Januar 2016')
            CalibParam_a = -0.01993 + exp(0.02355*(-208.09581 + ...
                obj.DeadTime)) - (0.17508/(2.94249+obj.DeadTime));
            CalibParam_b = 0.008119 - 4.82713e-10*obj.DeadTime + 2.21699e-12*obj.DeadTime.^2;
            CalibParam_c = -1.73724e-9 - 3.66366e-25*obj.DeadTime;
        % Februar 2016
        ...
        elseif strcmp(calib,'August 2016')
            CalibParam_a = -0.0244 + exp(0.02354*(-208.17143 + ...
                obj.DeadTime)) - (0.17481/(2.94231+obj.DeadTime));
            CalibParam_b = 0.00818 - 5.15725e-10*obj.DeadTime + 2.37128e-12*obj.DeadTime.^2;
            CalibParam_c = -1.85811e-9 + 4.96803e-26*obj.DeadTime;
        % Januar 2017
        elseif strcmp(calib,'Januar 2017')
            CalibParam_a = -0.02278 + exp(0.02354*(-208.19883 + ...
                obj.DeadTime)) - (0.17479/(2.94182+obj.DeadTime));
            CalibParam_b = 0.00818 - 5.25536e-10*obj.DeadTime + 2.41649e-12*obj.DeadTime.^2;
            CalibParam_c = -1.89342e-9 - 6.89439e-27*obj.DeadTime;
		% 30. Januar 2017
        elseif strcmp(calib,'30.Januar 2017')
            CalibParam_a = -0.02073 + exp(0.02353*(-208.20673 + ...
                obj.DeadTime)) - (0.17481/(2.94153+obj.DeadTime));
            CalibParam_b = 0.00818 - 5.29669e-10*obj.DeadTime + 2.43554e-12*obj.DeadTime.^2;
            CalibParam_c = -1.90829e-9 - 3.85551e-26*obj.DeadTime;
        elseif strcmp(calib,'08.Januar 2018')
            CalibParam_a = -0.01804 + exp(0.02354*(-208.29443 + ...
                obj.DeadTime)) - (0.17435/(2.94208+obj.DeadTime));
            CalibParam_b = 0.00816 - 5.50839e-9*obj.DeadTime + 2.53279e-12*obj.DeadTime.^2;
            CalibParam_c = -1.98461e-9 + 2.31062e-25*obj.DeadTime;
        end
...
%% ------------------------------------------------------------------------
    ...
    %% (* Durchlaufen der AScans *)
        ...
        %2. Energien berechnen
            ...
            if strcmp(calib,'08.Januar 2018')
                CalibParam_a = -0.01804 + exp(0.02354*(-208.29443 + ...
                    obj(j_c).DeadTime)) - (0.17435/(2.94208+obj(j_c).DeadTime));
                CalibParam_b = 0.00816 - 5.50839e-9*obj(j_c).DeadTime + 2.53279e-12*obj(j_c).DeadTime.^2;
                CalibParam_c = -1.98461e-9 + 2.31062e-25*obj(j_c).DeadTime;
            end
