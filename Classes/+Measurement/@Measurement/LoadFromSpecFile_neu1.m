    ...
    %% (* Durchlaufen der AScans *)
        ...
        for j_c = 1:Count_AScans
            ...
            if strcmp(Diffractometer.Name,'ETA3000')
                ...
                obj(j_c).ScanMode = ScanModeScan{1};
        ...
        %% (* Einlesen der Scan-Veränderlichen *)
            ...
            elseif strcmp(Diffractometer.Name,'LEDDI_KETEK_TWODET')
                AScanVariables = sscanf(Scan(Index_AScan_variables(j_c)-1,:),'%f',inf)';
            else
...
%% ------------------------------------------------------------------------
    ...
    function obj = AnalyzeMeshScan(Scan, Diffractometer,~)
    %% (* Anzahl und Index der AScans *)
...
%             Index_AScanDetector = repmat([1;2],length(Index_AScan)/2,1);
%         end
        % Muss noch an die richtige Stelle gebracht werden, da es bisher nicht richtig geschrieben wird. 
%         obj(1,j_c).addprop('NumberOfDetectors');
%         [obj(j_c).NumberOfDetectors] = Count_Detectors;
        end
    %% (* Scanbereiche des ascans und des Sin2Psi scans auslesen*)
        ScanParams = strsplit(Scan(1,:));
%         MotorNameMeshascan = ScanParams{4};
%         MotorMeshaScanRange = [str2double(ScanParams{5}) str2double(ScanParams{6})];
%         MotorMeshaScanStepNumber = str2double(ScanParams{7});
%         MotorMeshaScanStepSize = diff(MotorMeshaScanRange)/MotorMeshaScanStepNumber;
%         MotorMeshaScanSteps = linspace(MotorMeshaScanRange(1),MotorMeshaScanRange(2),MotorMeshaScanStepNumber+1);
% 
%         MotorNameMeshSin2Psi = ScanParams{8};
%         MotorMeshSin2PsiRange = [str2double(ScanParams{9}) str2double(ScanParams{10})];
%         MotorMeshSin2PsiStepNumber = str2double(ScanParams{11});
%         MotorMeshSin2PsiStepSize = diff(MotorMeshSin2PsiRange)/MotorMeshSin2PsiStepNumber;
%         MotorMeshSin2PsiSteps = linspace(MotorMeshSin2PsiRange(1),MotorMeshSin2PsiRange(2),MotorMeshSin2PsiStepNumber+1);
%         MotorMeshSin2PsiCountTime = str2double(ScanParams{12});
    %% (* Instanzen und allgemeine Eigenschaften *)
        %--> Prealloc der Messobjekte
        obj = Measurement.Measurement.CloneConstruction(...
...
%             [obj(j_c).ROI] = [obj(j_c).ChannelRange(1) obj(j_c).ChannelRange(2)];  
    ...
    %% (* Durchlaufen der AScans *)
        ...
        %% (* Einlesen der Scan-Veränderlichen *)
            ...
            elseif strcmp(Diffractometer.Name,'LEDDI_KETEK_TWODET')
                AScanVariables = sscanf(Scan(Index_MeshScan(j_c)-3,:),'%f',inf)';
%                 AScanVariables = sscanf(Scan(Index_MeshScan(j_c)-2,:),'%f',inf)';
            else
                AScanVariables = sscanf(Scan(Index_MeshScan(j_c)-1,:),'%f',inf)';
            end
...
%1. Channelrange (#@CHANN)
    ...
    %         obj(j_c).ChannelRange
            ...
            if isempty(Index_tmp)
                ...
                % Name (#R@OI BSI), wird später ergänzt
...
%             CalibParams1 = [-1.6821E-9; 0.0075204; -0.055875];
%             CalibParams2 = [-1.3358e-09; 0.0106; -0.0449];
%             
%             a = sscanf(Scan(1,:),'#S %d');
%             
% 
...
%                 a1 = a;
%                 a1
%                 Energies_tmp = polyval(CalibParams2,obj(j_c).ChannelRange(1):obj(j_c).ChannelRange(2));
%             else
%                 a2 = a;
%                 a2
%                 Energies_tmp = polyval(CalibParams1,obj(j_c).ChannelRange(1):obj(j_c).ChannelRange(2));
%             end
...
%             if strcmp(ScanType,{'mesh'})
%                 obj(i_c).twotheta = 0;
%                 obj(i_c).SCSAngles.psi = asind(sqrt(obj(i_c).Motors_all.Sin2Psi));
...
%                                 end
                        ...
                        if strcmp(Diffractometer.Name,'LEDDI_KETEK_TWODET')
                            if obj(1).NumberOfDetectors ~= 1
                                ...
                                % ist der aeußere mca(3) und der innere mca(2).
    %                             if obj(1).Motors_all.Det1 < obj(1).Motors_all.Det2
    %                                 twothetatmp = abs([obj(1).Motors_all.Det1 obj(1).Motors_all.Det2]);
    %                             elseif obj(1).Motors_all.Det1 > obj(1).Motors_all.Det2
                                twothetatmp = abs([obj(1).Motors_all.Det2 obj(1).Motors_all.Det1]);
%                                 twothetatmp = abs([obj(1).Motors_all.Det1 obj(1).Motors_all.Det2]);
    %                             end
                                twothetatmp = repmat(twothetatmp,1,length(obj));
