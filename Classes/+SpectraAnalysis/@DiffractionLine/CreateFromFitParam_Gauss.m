...
function obj = CreateFromFitParam_Gauss(FitParam,CI,Measurement_in)
...
%% (* Erzeugen des Objektes *)
    ...
    for i_c = 1:size(FitParam,1) 
        ...
        obj(i_c).IntegralWidth = sqrt(2*pi) * FitParam(i_c,3);
        %FWHM ermitteln
        obj(i_c).FWHM = 2 * sqrt(2 * log(2)) * FitParam(i_c,3);
