	...
	methods (Access = private)
		...
		% und die Dependencies der Fit-Funktion.
			...
			for i = 1:size(obj.fitParamTable, 1)
				...
				% Abbruch, wenn ein Parameter kein Pattern hat
				if (isempty(dep))
				...
				for paramInd = obj.fitParamTable{i, 3}
					...
					jSub = sparse(repmat(jSub(:), [1, prod(obj.fitParamTable{i, 4})]));
...
%% Fit-Methoden
	...
	methods (Access = public)
		...
		% Hier koennen bestimmte Fit-Optionen gesetzt werden. Die Eingabe
		% erfolgt nach dem Property-Schema. Es gibt folgende Optionen
		% (Standardwert in Klammern):
		%	Display (true) - Ausgabe von Fitinformationen
		%	ParameterDiag (true) - Analyse der Fitparameter
		%	ComputeFitErrors (true) - Berechnet nach dem Fit die Fehler
		%	UseWeighting (true) - Gewichtung der Y-Daten mit ihrer Wurzel
		%	Scaling (1) - Skalierungsfaktor der Y-Daten
			...
			ip.addParamValue('ComputeFitErrors', true, @(x)validateattributes(x, {'logical'}, {'scalar'}));
			ip.addParamValue('UseWeighting', true, @(x)validateattributes(x, {'logical'}, {'scalar'}));
			ip.addParamValue('Scaling', 1, @(x)validateattributes(x, {'double'}, {'scalar'}));
			ip.parse(varargin{:});
			
			% Nur die uebergebenen Werte sollen geaendert werden
			if (isempty(obj.fitOptions))
				
				obj.fitOptions = ip.Results;
			else
				
				for opt = setdiff(fieldnames(ip.Results), ip.UsingDefaults)
					
					obj.fitOptions.(opt{1}) = ip.Results.(opt{1});
				end
			end
		end
		...
		function [fitOutput, resnorm] = executeFit(obj, fitContainer)
			...
			% Skalierung
			dataYWeight = dataYWeight / obj.getFitOptions().Scaling;
% 			assignin('base','dataYWeightFit',dataYWeight)
%             assignin('base','dataXFit',dataX)
%             assignin('base','dataYFit',dataY)
			% lsqcurvefit ausfuehren
			% Da die Daten im Container von beliebiger Dimension sein
			% koennen, werden sie hier zu einem Vektor gemacht
			[P,resnorm,residual,~,fitOutput,~,jacobian] = lsqcurvefit(@(p, x)lsqFitFunc(obj, p, x) ./ dataYWeight,...
				startValues, dataX, dataY ./ dataYWeight,...
			...
			if (obj.getFitOptions().ComputeFitErrors)
				...
				% wichtig ist, dass mal die Sparse-Matrix zunaechst in eine
				% vollbesetzte Matrix umwandelt, andernfalls gibt es
				% erhebliche Performance-Einbussen
				[~, SE] = util.nlparci_2014(P, residual, 'jacobian', full(jacobian));
			else
	...
	methods (Access = private)
		...
		% Ermittelt aus "fitOptions" die Eingabestruktur fuer lsqcurvefit.
			...
			lsqcfOpt.TolFun =  1.0e-008;
			lsqcfOpt.TolX =  1.0e-006;
		end
