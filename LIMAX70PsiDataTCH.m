function [Psidata] = LIMAX70PsiDataTCH(DiffractionLines)
    for k = 1:size(DiffractionLines,1)
        for l = 1:size(DiffractionLines{k},2)
            ...
            Psidata.eta(k,l) = cellstr(num2str(DiffractionLines{k,1}(l).SCSAngles.eta,'%.3f'));
            ...
            Psidata.x_achse(k,l) = cellstr(num2str(DiffractionLines{k,1}(l).Motors.XE,'%.4f'));
            Psidata.y_achse(k,l) = cellstr(num2str(DiffractionLines{k,1}(l).Motors.YE,'%.4f'));
            Psidata.z_achse(k,l) = cellstr(num2str(DiffractionLines{k,1}(l).Motors.ZE,'%.4f'));
