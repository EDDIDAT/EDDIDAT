function [Psidata] = LEDDIPsiDataTCHSaveMod(DiffractionLines)
    for k = 1:size(DiffractionLines,1)
        for l = 1:size(DiffractionLines{k},2)
            ...
            Psidata.alpha(k,l) = DiffractionLines{k,1}(l).SCSAngles.alpha;
            Psidata.beta(k,l) = DiffractionLines{k,1}(l).SCSAngles.beta;
