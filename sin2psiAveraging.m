...
if strcmp(h.Measurement(1).Sample.Materials.CrystalStructure,'fcc') || strcmp(h.Measurement(1).Sample.Materials.CrystalStructure,'bcc') && size(h.ParamsToFit(1).LatticeSpacing,2) ~= 1
    ...
    if isfield(h,'idxhklPeaktable')
        ...
        Params_tmp = h.Params;
        assignin('base','Params_tmp',Params_tmp)
...
%     ax.YLim = [floor((min(cellfun(@min,daverage))-0.0002)./0.0001).*0.0001 ceil((max(cellfun(@max,daverage))+0.0002)/0.0001).*0.0001];
    ...
    % Export File
    for k = 1:size(daverage,2)
        dataexport = [sin2psiinterpol daverage{k}' derraverage{k}'];
        FileNameExport = sprintf([strrep(h.Measurement(1).MeasurementSeries,' ',''),'_',h.Sample.Materials.Name,'_dspacing_averaged_phi',num2str(k)]);
        Path = fullfile(h.PathName,Folder);
        writematrix(dataexport,[Path,FileNameExport],'Delimiter','tab')
        FileNameRegressLine = sprintf([strrep(h.Measurement(1).MeasurementSeries,' ',''),'_',h.Sample.Materials.Name,'_regressline_phi',num2str(k)]);
        if regresscoeff{k}(2,1) < 0
            regressline = ['y = ',num2str(regresscoeff{k}(1,1)),num2str(regresscoeff{k}(2,1)),'*x'];
            writematrix(regressline,[Path,FileNameRegressLine],'Delimiter','tab')
        else
            regressline = ['y = ',num2str(regresscoeff{k}(1,1)),'+',num2str(regresscoeff{k}(2,1)),'*x'];
            writematrix(regressline,[Path,FileNameRegressLine],'Delimiter','tab')
        end
