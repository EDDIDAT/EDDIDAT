function [Psidata] = EDDIPsiDataTCH(DiffractionLines)
    for k = 1:size(DiffractionLines,1)
        for l = 1:size(DiffractionLines{k},2)
            ...
            Psidata.x_achse(k,l) = cellstr(num2str(DiffractionLines{k,1}(l).Motors.x_achse,'%.4f'));
            Psidata.y_achse(k,l) = cellstr(num2str(DiffractionLines{k,1}(l).Motors.y_achse,'%.4f'));
            Psidata.z_achse(k,l) = cellstr(num2str(DiffractionLines{k,1}(l).Motors.z_achse,'%.4f'));
